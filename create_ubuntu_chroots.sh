...
create_build_scripts () {
	...
	echo 'wget -O faudio.tar.gz https://github.com/FNA-XNA/FAudio/archive/20.12.tar.gz' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'wget -O vulkan-loader.tar.gz https://github.com/KhronosGroup/Vulkan-Loader/archive/v1.2.162.tar.gz' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'wget -O vulkan-headers.tar.gz https://github.com/KhronosGroup/Vulkan-Headers/archive/v1.2.162.tar.gz' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'wget -O spirv-headers.tar.gz https://github.com/KhronosGroup/SPIRV-Headers/archive/1.5.4.raytracing.fixed.tar.gz' >> "${MAINDIR}"/prepare_chroot.sh
	...
	echo 'git clone git://source.winehq.org/git/vkd3d.git' >> "${MAINDIR}"/prepare_chroot.sh
	...
	echo 'cmake ../FAudio-20.12 && make -j$(nproc) && make install' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cd ../ && rm -r build && mkdir build && cd build' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cmake ../Vulkan-Headers-1.2.162 && make -j$(nproc) && make install' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cd ../ && rm -r build && mkdir build && cd build' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cmake ../Vulkan-Loader-1.2.162 && make -j$(nproc) && make install' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cd ../ && rm -r build && mkdir build && cd build' >> "${MAINDIR}"/prepare_chroot.sh
	echo 'cmake ../SPIRV-Headers-1.5.4.raytracing.fixed && make -j$(nproc) && make install' >> "${MAINDIR}"/prepare_chroot.sh
